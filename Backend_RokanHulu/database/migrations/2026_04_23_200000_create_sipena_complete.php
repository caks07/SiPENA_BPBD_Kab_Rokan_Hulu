...
return new class extends Migration {
    public function up(): void
    {
        ...
        // Enums
        DB::statement("DROP TYPE IF EXISTS role_name CASCADE");
        DB::statement("CREATE TYPE role_name AS ENUM('admin','pimpinan','operator')");
        DB::statement("DROP TYPE IF EXISTS sumber_laporan CASCADE");
        DB::statement("CREATE TYPE sumber_laporan AS ENUM('trc','kecamatan','admin','import')");
        DB::statement("DROP TYPE IF EXISTS sumber_koordinat CASCADE");
        DB::statement("CREATE TYPE sumber_koordinat AS ENUM('titik_peta','default_kecamatan','manual')");
        DB::statement("DROP TYPE IF EXISTS jenis_bencana CASCADE");
        DB::statement("CREATE TYPE jenis_bencana AS ENUM('banjir','banjir_bandang','tanah_longsor','cuaca_ekstrim','kekeringan','karhutla','wabah','gempa_bumi','konflik_sosial')");
        DB::statement("DROP TYPE IF EXISTS status_laporan CASCADE");
        DB::statement("CREATE TYPE status_laporan AS ENUM('baru','diverifikasi','diproses','selesai','dibatalkan')");

        // OPT tables
        $optTables = [
            'opt_kondisi_cuaca','opt_fasilitas_umum','opt_kebutuhan_logistik',
            'opt_banjir_penyebab','opt_banjir_ketinggian','opt_banjir_kondisi_air',
            'opt_bandang_kecepatan_air','opt_bandang_material_terbawa','opt_bandang_kondisi_arus','opt_bandang_kerusakan_infrastruktur',
            'opt_longsor_penyebab','opt_longsor_jenis_lokasi','opt_longsor_akses_transportasi','opt_longsor_material','opt_longsor_potensi_susulan',
            'opt_cuaca_fenomena','opt_cuaca_dampak_pohon','opt_cuaca_kerusakan_bangunan',
            'opt_kekeringan_sektor','opt_kekeringan_kondisi_air','opt_kekeringan_durasi','opt_kekeringan_potensi_risiko','opt_kekeringan_upaya_masyarakat',
            'opt_karhutla_kondisi_api','opt_karhutla_jenis_lahan','opt_karhutla_pemilik_lahan','opt_karhutla_jarak_pemukiman','opt_karhutla_sumber_air','opt_karhutla_akses_lokasi',
            'opt_wabah_jenis_penyakit','opt_wabah_sebaran','opt_wabah_fasilitas_kesehatan','opt_wabah_kondisi_sanitasi',
            'opt_gempa_durasi','opt_gempa_kekuatan','opt_gempa_dampak_struktural','opt_gempa_kerusakan_jalan','opt_gempa_potensi_susulan','opt_gempa_kondisi_warga',
            'opt_konflik_sifat','opt_konflik_aktor','opt_konflik_pemicu','opt_konflik_jumlah_terlibat','opt_konflik_kerusakan_materil','opt_konflik_aparat',
        ];
        foreach ($optTables as $t) {
            DB::statement("DROP TABLE IF EXISTS {$t} CASCADE");
            DB::statement("CREATE TABLE {$t} (
        ...
        // kecamatan
        DB::statement("DROP TABLE IF EXISTS kecamatan CASCADE");
        DB::statement("CREATE TABLE kecamatan (
        ...
        // users (override Laravel default)
        ...
        // laporan_bencana
        DB::statement("DROP TABLE IF EXISTS laporan_bencana CASCADE");
        DB::statement("CREATE TABLE laporan_bencana (
            id bigserial PRIMARY KEY,
        ...
        DB::statement("CREATE INDEX IF NOT EXISTS idx_laporan_jenis ON laporan_bencana(jenis_bencana)");
        DB::statement("CREATE INDEX IF NOT EXISTS idx_laporan_kecamatan ON laporan_bencana(kecamatan_id)");
        DB::statement("CREATE INDEX IF NOT EXISTS idx_laporan_status ON laporan_bencana(status)");
        DB::statement("CREATE INDEX IF NOT EXISTS idx_laporan_waktu ON laporan_bencana(waktu_kejadian)");

        // korban_bencana
        DB::statement("DROP TABLE IF EXISTS korban_bencana CASCADE");
        DB::statement("CREATE TABLE korban_bencana (
        ...
        // kerusakan_bencana
        DB::statement("DROP TABLE IF EXISTS kerusakan_bencana CASCADE");
        DB::statement("CREATE TABLE kerusakan_bencana (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // kerusakan pivot tables
        DB::statement("DROP TABLE IF EXISTS kerusakan_fasilitas_umum CASCADE");
        DB::statement("CREATE TABLE kerusakan_fasilitas_umum (
        ...
        DB::statement("DROP TABLE IF EXISTS kerusakan_kebutuhan_logistik CASCADE");
        DB::statement("CREATE TABLE kerusakan_kebutuhan_logistik (
            laporan_id bigint REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // laporan_foto
        DB::statement("DROP TABLE IF EXISTS laporan_foto CASCADE");
        DB::statement("CREATE TABLE laporan_foto (
            id bigserial PRIMARY KEY,
        ...
        // laporan_log
        DB::statement("DROP TABLE IF EXISTS laporan_log CASCADE");
        DB::statement("CREATE TABLE laporan_log (
            id bigserial PRIMARY KEY,
            laporan_id bigint NOT NULL REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // Detail tables per bencana
        // detail_banjir
        DB::statement("DROP TABLE IF EXISTS detail_banjir CASCADE");
        DB::statement("CREATE TABLE detail_banjir (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        DB::statement("DROP TABLE IF EXISTS detail_banjir_bandang CASCADE");
        DB::statement("CREATE TABLE detail_banjir_bandang (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
            kondisi_cuaca_id smallint REFERENCES opt_kondisi_cuaca(id),
        ...
        DB::statement("DROP TABLE IF EXISTS detail_tanah_longsor CASCADE");
        DB::statement("CREATE TABLE detail_tanah_longsor (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
            kondisi_cuaca_id smallint REFERENCES opt_kondisi_cuaca(id),
        ...
        DB::statement("DROP TABLE IF EXISTS detail_cuaca_ekstrim CASCADE");
        DB::statement("CREATE TABLE detail_cuaca_ekstrim (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // detail_kekeringan
        DB::statement("DROP TABLE IF EXISTS detail_kekeringan CASCADE");
        DB::statement("CREATE TABLE detail_kekeringan (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // detail_karhutla
        DB::statement("DROP TABLE IF EXISTS detail_karhutla CASCADE");
        DB::statement("CREATE TABLE detail_karhutla (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // detail_wabah
        DB::statement("DROP TABLE IF EXISTS detail_wabah CASCADE");
        DB::statement("CREATE TABLE detail_wabah (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        // detail_gempa
        DB::statement("DROP TABLE IF EXISTS detail_gempa_bumi CASCADE");
        DB::statement("CREATE TABLE detail_gempa_bumi (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
        ...
        DB::statement("DROP TABLE IF EXISTS detail_konflik_sosial CASCADE");
        DB::statement("CREATE TABLE detail_konflik_sosial (
            laporan_id bigint PRIMARY KEY REFERENCES laporan_bencana(id) ON DELETE CASCADE,
            ...
            aparat_id smallint REFERENCES opt_konflik_aparat(id),
            aparat_lain text,
            created_at timestamp NOT NULL DEFAULT now(),
            updated_at timestamp NOT NULL DEFAULT now()
        )");

        // cache, jobs tables for Laravel
        DB::statement("DROP TABLE IF EXISTS cache CASCADE");
        DB::statement("CREATE TABLE cache (key varchar(255) PRIMARY KEY, value text NOT NULL, expiration int NOT NULL)");
        DB::statement("DROP TABLE IF EXISTS cache_locks CASCADE");
        DB::statement("CREATE TABLE cache_locks (key varchar(255) PRIMARY KEY, owner varchar(255) NOT NULL, expiration int NOT NULL)");
        ...
        DB::statement("CREATE TABLE jobs (
            ...
            attempts smallint NOT NULL, reserved_at int, available_at int NOT NULL,
            created_at int NOT NULL
        )");
        DB::statement("DROP TABLE IF EXISTS failed_jobs CASCADE");
        DB::statement("CREATE TABLE failed_jobs (
            id bigserial PRIMARY KEY, uuid varchar(255) NOT NULL UNIQUE,
            connection text NOT NULL, queue text NOT NULL, payload text NOT NULL,
            exception text NOT NULL, failed_at timestamp NOT NULL DEFAULT now()
        )");
        ...
        DB::statement("CREATE TABLE job_batches (
            ...
            total_jobs int NOT NULL, pending_jobs int NOT NULL, failed_jobs int NOT NULL,
            failed_job_ids text NOT NULL, options text, cancelled_at int,
            created_at int NOT NULL, finished_at int
        )");
