...
/**
 * Migration Baseline Final — SiPENA BPBD Kab. Rokan Hulu
 *
 * Dibuat: 2026-05-06
 * Menggantikan 11 migration lama yang sudah tambal-sulam.
 * Sumber kebenaran: database existing (bpbd_baseline_schema.sql + semua ALTER).
 *
 * State final:
 *  - status_laporan ENUM: {siaga1, siaga2, siaga3, selesai}
 *  - laporan_bencana.status default = 'siaga1'
 *  - laporan_bencana.severity_level default = 1
 *  - laporan_bencana.catatan_update (text, nullable)
 *  - detail_konflik_sosial: aparat_ids smallint[] (bukan aparat_id)
 *  - laporan_log: field_changed, old_value, new_value (json nullable)
 */
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // FINAL: siaga1/siaga2/siaga3/selesai (bukan baru/diverifikasi/diproses)
        DB::statement("DROP TYPE IF EXISTS status_laporan CASCADE");
        DB::statement("CREATE TYPE status_laporan AS ENUM('siaga1','siaga2','siaga3','selesai')");

        // ─── Option Tables (46 tabel) ────────────────────────────────────────
        $optTables = [
            'opt_kondisi_cuaca',
            'opt_fasilitas_umum',
        ...
        ");
        DB::statement("CREATE INDEX idx_laporan_jenis     ON laporan_bencana(jenis_bencana)");
        DB::statement("CREATE INDEX idx_laporan_kecamatan ON laporan_bencana(kecamatan_id)");
        DB::statement("CREATE INDEX idx_laporan_status    ON laporan_bencana(status)");
        DB::statement("CREATE INDEX idx_laporan_waktu     ON laporan_bencana(waktu_kejadian)");
        ...
        // ─── pivot: kerusakan_fasilitas_umum ────────────────────────────────
        ...
        // ─── pivot: kerusakan_kebutuhan_logistik ─────────────────────────────
        ...
        // ─── laporan_log (dengan kolom audit JSON) ───────────────────────────
        ...
        ");
        DB::statement("CREATE INDEX pat_tokenable_idx ON personal_access_tokens(tokenable_type, tokenable_id)");
        DB::statement("CREATE INDEX pat_expires_idx   ON personal_access_tokens(expires_at)");
        ...
        // ─── detail_konflik_sosial (aparat_ids = array, bukan FK) ────────────
    ...
    {
        // Nuclear rollback — wipes entire public schema
        DB::statement("DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO public");
    }
};
