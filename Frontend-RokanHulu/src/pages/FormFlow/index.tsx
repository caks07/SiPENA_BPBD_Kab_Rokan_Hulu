...
export default function FormFlow() {
  ...
  return (
    <div className="min-h-screen pb-32" style={{ fontFamily: "Inter, sans-serif", background: "#f9f9f9", backgroundImage: "radial-gradient(circle at 2px 2px, #e2e2e2 1px, transparent 0)", backgroundSize: "40px 40px" }}>
      {/* TopNav */}
      <nav style={{ background: "rgba(15,23,42,0.85)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.1)" }}
      ...
      <main className="pt-24 px-4 max-w-4xl mx-auto space-y-6">
        {/* Step Badge + Info */}
        <div className="mb-6 p-4 rounded-lg border border-slate-200 flex items-center gap-3 bg-white/80">
        ...
        <div className="flex items-center gap-1 mb-8">
          {STEPS.map((label, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className={`h-1 w-full rounded-full ${step > i ? "bg-amber-500" : "bg-slate-200"}`}
