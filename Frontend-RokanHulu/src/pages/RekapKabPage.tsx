...
export default function RekapKabPage() {
  ...
  const handleExportExcel = async () => {
    setIsExporting(true);
    setExportLoadingText("Mengekspor Excel...");
    try {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <CustomSelect
                      ...
                      options={[
                        ...
                        ...kecamatans.map((k) => ({ value: String(k.id), label: k.nama_kecamatan }))
                      ]}
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <CustomSelect
                      value={filterSiaga}
                      onChange={setFilterSiaga}
                      borderClass="border border-slate-200"
                      placeholder="Semua Level"
                      options={[
                        { value: "semua", label: "Semua Level" },
                        { value: "siaga1", label: "Siaga 1" },
                        { value: "siaga2", label: "Siaga 2" },
                        { value: "siaga3", label: "Siaga 3" },
                ...
                <tbody className="divide-y divide-slate-100">
                  {isLoading ? (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/40 transition-colors group">
                      ...
                      <td className="px-4 py-3 max-w-[160px] truncate text-sm text-slate-500" title={item.lokasi_text ?? ""}>{item.lokasi_text ?? "-"}</td>
                      <td className="px-4 py-3 max-w-[150px] truncate text-sm text-slate-700" title={item.nama_pelapor ?? ""}>{item.nama_pelapor ?? "-"}</td>
                      <td className="px-4 py-3">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`inline-flex px-2.5 py-1 rounded-full text-[9px] font-black uppercase leading-none ${SIAGA_CLS[item.status] ?? "bg-slate-200 text-slate-600"}`}>
                          {SIAGA_LABEL[item.status] ?? item.status}
                        </span>
                      </div>
                      ...
                      {/* 3-Column Info */}
                      <div className="grid grid-cols-3 gap-2 border-b border-slate-100 pb-3 mb-3 text-center sm:text-left">
                        <div className="min-w-0">
                          <div className="flex items-center justify-center sm:justify-start gap-1 text-slate-400 mb-0.5">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-wider">Kecamatan</span>
                          </div>
                          <span className="font-bold text-slate-700 text-xs block truncate" title={item.nama_kecamatan ?? ""}>
                            {item.nama_kecamatan ?? "-"}
                          </span>
                        </div>
                        <div className="min-w-0">
                      ...
                      {/* Bottom actions row */}
                      <div className="grid grid-cols-3 gap-2 sm:flex sm:items-center sm:gap-1.5 pt-2 border-t border-slate-100 no-capture">
                        {role !== "pimpinan" && (
                          <button
                        ...
                        <button
                          onClick={() => handleExportPdfItem(item.id)}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 px-1 text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-200 rounded-lg hover:bg-rose-100 active:scale-95 transition-all"
                        >
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 border border-slate-200 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center"
                  >
        ...
        <div className="no-capture fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-slate-900/70 backdrop-blur-md text-white animate-in fade-in duration-200">
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-sm w-[calc(100%-32px)] shadow-2xl flex flex-col items-center text-center">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-6">
              Sedang menyiapkan tata letak beresolusi tinggi untuk mengunduh gambar rekapitulasi...
            </p>
      ...
      {exportLoadingText && (
        <div className="no-capture fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-slate-900/70 backdrop-blur-md text-white animate-in fade-in duration-200">
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-sm w-[calc(100%-32px)] shadow-2xl flex flex-col items-center text-center">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-6">
              Sedang memproses dokumen Anda. Silakan tunggu sebentar hingga proses selesai...
            </p>
            <div className="flex items-center gap-2 text-xs font-semibold text-amber-400 bg-amber-950/40 border border-amber-900/30 px-4 py-2.5 rounded-full">
              <div className="animate-spin w-3.5 h-3.5 rounded-full border-2 border-amber-400 border-t-transparent" />
